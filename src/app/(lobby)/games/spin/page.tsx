...
const WHEEL_COLORS = [
  ...
  const WHEEL_SEGMENTS = useMemo(() => {
    return roomData.players.map((player: string, index: number) => {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-purple-200 to-purple-400 flex items-center justify-center p-4">
            ...
            >
              {WHEEL_SEGMENTS.map((segment, index) => {
                ...
                return (
                  <div
                    key={index}
                    className="absolute font-bold text-sm sm:text-lg z-10" // <-- Nэмэлт өөрчлөлт: z-index-ийг нэмсэн
                    style={{
                      left: '50%',
                      top: '50%',
                      transform: `rotate(${textRotation}deg) translateY(-100px) translateX(-50%)`, // <-- Байршлыг илүү тодорхой болгосон
                      transformOrigin: 'center',
                      color: segment.textColor,
                      // background: 'rgba(0,0,0,0.5)', // Дибаг хийхэд зориулсан
                      // padding: '5px' // Дибаг хийхэд зориулсан
                    }}
                  >
                    <span 
                      style={{ 
                        transform: `rotate(-${textRotation}deg)`,
                        textShadow: '2px 2px 4px rgba(0,0,0,0.8)' // <-- Илүү тод болгосон
                      }}>
                      {segment.text}
                    </span>
                  </div>
     ...
     <div className="text-center p-8 bg-gradient-to-r from-purple-50 to-indigo-50 dark:from-purple-900/20 dark:to-indigo-900/20 rounded-2xl border-2 border-purple-200 dark:border-purple-700 shadow-lg">
      ...
      <p className="text-xl font-bold text-purple-700 dark:text-purple-300 bg-white dark:bg-slate-800 px-6 py-3 rounded-full shadow-md">
        {winner}
      </p>
    </div>
          )}
